...
class SpiceDataProcessor:
    ...
    def __init__(self, base_path="spice_data"):
        ...
        self.STATIC_KERNELS = [
            ...
            "MEX/kernels/sclk/MEX_250724_STEP.TSC",
    ...
    def get_spice_data(self, utc_time):
        """Get SPICE data for all bodies at given UTC time with correct quaternion calculations."""
        ...
        try:
            ...
            q_hrsc_j2000 = spice.m2q(att_hrsc)
            
            # Calculate HRSC quaternion for Blender (with proper axis conversion)
            q_hrsc_blender = self._calculate_hrsc_blender_quaternion(q_hrsc_j2000)

            # Sun lamp orientation calculation (corrected version)
            ...
            results = {
                ...
                'hrsc':   {'position': np.array([0, 0, 0]).tolist(), 'quaternion': q_hrsc_blender.tolist()},
                # Additional debug information
                'hrsc_j2000_quaternion': q_hrsc_j2000.tolist(),  # Original SPICE quaternion
                'distances': {
                    'sun_to_mars': float(np.linalg.norm(pos_sun - pos_mars)),
                    'sun_to_phobos': float(np.linalg.norm(pos_sun - pos_phobos)),
                    'sun_to_deimos': float(np.linalg.norm(pos_sun - pos_deimos)),
                    'sun_to_hrsc': float(np.linalg.norm(pos_sun))
                }
            }
            
            # Print debug information
            print(f"HRSC J2000 Quaternion: {q_hrsc_j2000}")
            print(f"HRSC Blender Quaternion: {q_hrsc_blender}")
            print(f"Sun Lamp Quaternion: {q_sun_lamp}")
            print(f"Distance from Sun to Mars [km]: {results['distances']['sun_to_mars']}")
            print(f"Distance from Sun to Phobos [km]: {results['distances']['sun_to_phobos']}")
            print(f"Distance from Sun to HRSC [km]: {results['distances']['sun_to_hrsc']}")
            ...
            results = {
                ...
                'hrsc_j2000_quaternion': [1.0, 0.0, 0.0, 0.0],
                'distances': {
                    'sun_to_mars': 0.0,
                    'sun_to_phobos': 0.0,
                    'sun_to_deimos': 0.0,
        ...
        # Quaternion utility functions to add to SpiceDataProcessor class
    def _q_mult(self, q1: np.ndarray, q2: np.ndarray) -> np.ndarray:
        """
        Hamilton quaternion product q = q1 * q2 (Shoemake 1985).
    ...
    def _q_conj(self, q: np.ndarray) -> np.ndarray:
        """
        Quaternion conjugate q* (inverse for unit quaternions).
    ...
    def _q_rotate(self, q: np.ndarray, v: np.ndarray) -> np.ndarray:
        """
        Rotate 3-vector v by quaternion q using v' = q v q* (Diebel 2006).
        ...
        return self._q_mult(self._q_mult(q, q_v), self._q_conj(q))[1:]

    def _calculate_hrsc_blender_quaternion(self, hrsc_j2000_quaternion):
        """
        Calculate HRSC camera quaternion for Blender coordinate system.
        
        HRSC Boresight Quaternion Calculation for Blender:
        Starting from the attitude quaternion of the Mars Express HRSC camera
        (Q_j2000, given from SPICE, J2000 → HRSC) we derive the quaternion
        that Blender expects (Q_blender). Blender's camera looks along −Z,
        whereas HRSC's boresight (optical axis) is +Z, so we compose a 180°
        rotation about the camera X-axis with the original attitude quaternion.
        
        Args:
            hrsc_j2000_quaternion: HRSC attitude quaternion from SPICE (J2000 → HRSC)
            
        Returns:
            numpy.ndarray: Quaternion for Blender coordinate system [w, x, y, z]
        """
        
        # Input quaternion from SPICE (J2000 ➜ HRSC)
        Q_j2000 = hrsc_j2000_quaternion
        
        # Define 180° rotation about +X axis (maps +Z ➜ −Z)
        # This transforms HRSC's +Z boresight to Blender's -Z camera direction
        Q_flip = np.array([0.0, 1.0, 0.0, 0.0])  # cos(π/2)=0, sin(π/2)*axis = (1,0,0)
        
        # Compose: first flip local axes, then apply attitude
        Q_blender = self._q_mult(Q_j2000, Q_flip)
        
        # Optional: negate so w > 0 (quaternions q and −q represent same rotation)
        if Q_blender[0] < 0:
            Q_blender *= -1
    ...
    def _calculate_sun_alignment_quaternion(self, pos_sun):
        """
        Calculate sun lamp orientation quaternion for Blender alignment.
        
        This function computes the rotation quaternion required to align a Sun lamp
        in Blender with a specified light direction (-pos_sun).
        
        The calculation proceeds as follows:
        1. Retrieve the raw position vector from observer to the Sun.
        2. Normalize this vector to obtain a unit direction vector.
        3. Determine the incoming light direction (from the Sun to observer),
        which is the negative of the first vector.
        4. Compute the rotation between Blender's default lamp local –Z axis
        ([0, 0, –1]) and this target light direction.
        5. Return the quaternion that represents this rotation.
        
        Args:
            pos_sun: Position vector from observer to Sun (3D numpy array)
            
        Returns:
            numpy.ndarray: Normalized quaternion [w, x, y, z]
        """
        
        # --- Input Data ---
        # Step 1: Raw vector obtained from SPICE (Observer → Sun, J2000 frame)
        observer_to_sun_vec = pos_sun
        
        # Handle edge case of zero vector
        norm = np.linalg.norm(observer_to_sun_vec)
        if norm == 0:
            return np.array([1, 0, 0, 0])  # Identity quaternion
        
        # Step 2: Obtaining a unit vector by normalizing the vector
        unit_vec_observer_to_sun = observer_to_sun_vec / norm
        
        # Step 3: Determine the direction of the incoming light (Sun → Observer)
        # This is the unit vector from the Sun to Observer and represents the target the lamp will align with.
        light_direction_vec = -unit_vec_observer_to_sun
        
        # --- Quaternion Calculation ---
        # Step 4: Define the starting and target vectors
        # The Sun lamp in Blender emits its light along the local -Z axis.
        # Default (unrotated) direction is [0, 0, -1].
        start_vec = np.array([0.0, 0.0, -1.0])
        target_vec = light_direction_vec
        
        # Compute the quaternion that turns the vector 'start_vec' into the vector 'target_vec'
        dot = np.dot(start_vec, target_vec)
        
        # If the vectors are already in opposite directions (180 degree rotation)
        if dot < -0.999999:
            # Rotate 180 degrees around a perpendicular axis
            ...
            # 180 degrees turning makes w=0
            return np.array([0.0] + list(rotation_axis))
        
        # Standard quaternion calculation
        cross_product = np.cross(start_vec, target_vec)
        w_component = math.sqrt((1.0 + dot) * 2.0)
        xyz_divisor = w_component
        
        w = xyz_divisor / 2.0
        x = cross_product[0] / xyz_divisor
        y = cross_product[1] / xyz_divisor
        z = cross_product[2] / xyz_divisor
        
        alignment_quaternion = np.array([w, x, y, z])
        
        # Normalize the results
        final_quaternion = alignment_quaternion / np.linalg.norm(alignment_quaternion)
