...
PDS_IMAGES = [
    ...
    {
        "file": "PDS_Phobos_KnownK/HF102_0005_SR2.IMG",
        "camera": "hrsc",
        "calibrated": True,
        "exposure_ms": 20.16,
        "lines": 1018,
...
# -- Atmosphere toggle ------------------------------------------------------
# Set False to disable Mars atmosphere creation entirely (skips volume render). PDS_Phobos_KnownK/HF102_0005_SR2.IMG
...
# -- Noise Pipeline ----------------------------------------------------------
HOT_PIXEL_SIGMA = 2.23       # Sigma multiplier for float data (OSIRIS) 
...
METRIC_WEIGHTS = {
    "nrmse": 0.15,   # Pixel-level error (lower=better)
    "ssim":  0.35,   # Structural similarity (higher->1-val) — dominant
    "emd":   0.10,   # Earth Mover's Distance — DN position+shape (lower=better)
    "ncc":   0.10,   # Normalized cross-correlation (higher->1-val)
    "gmsd":  0.15,   # Gradient similarity (lower=better)
    "lpips": 0.15,   # Perceptual similarity (lower=better)
}
