...
class CompactCORTOValidator:
    ...
    def _apply_corto_pipeline_and_template_matching(self, real_img, synthetic_imgs, synthetic_img_paths):
        """Combined CORTO pipeline with gamma-aware processing
        ...
        # Apply L2-style gamma correction to match real camera data
        gamma_value = 0.45  # HRSC-SRC characteristic
        max_physical_value = 2**14 - 1
        scale_factor = 255.0 / max_physical_value
        
        # ❌ ESKİ KOD (her görüntü için ayrı min-max):
        # real_normalized = real_img if real_img.dtype == np.uint8 else ((real_img - real_img.min()) / (real_img.max() - real_img.min()) * 255.0).astype(np.uint8)
        
        # ✅ YENİ KOD (global scale):
        ...
        # IMPORTANT: Post-processor'da stretch'i disable et
        try:
            # Post-processor'ın kendi normalization'ını bypass et
            real_processed, _ = self.post_processor.process_image_label_pair_no_stretch(real_rgb, labels.copy())
        except AttributeError:
            # Fallback: mevcut post-processor kullan ama sonucu scale'le
        ...
        template_results = []
        brightness_differences = []  # 🔧 YENİ: Parlaklık farkı tracking
        
        # Process synthetic images with same gamma correction
        for i, (synthetic_img, img_path) in enumerate(zip(synthetic_imgs, synthetic_img_paths)):
            ...
            try:
                synthetic_processed, _ = self.post_processor.process_image_label_pair_no_stretch(synthetic_rgb, labels.copy())  # ✅ synthetic_rgb artık tanımlı
            except AttributeError:
            ...
            # 🔧 YENİ: Mutlak parlaklık farkı hesabı
            ...
            # Template matching with improved images
            correlation = cv2.matchTemplate(real_masked, synthetic_masked, cv2.TM_CCOEFF_NORMED)
            ...
            # CORTO Equation (2) NRMSE - DEĞIŞMEZ
            mse = np.mean((real_masked.astype(np.float64) - synthetic_masked.astype(np.float64)) ** 2)
            d = real_masked.size  # total number of pixels
            nrmse = np.sqrt(mse) / np.sqrt(d)  # CORTO Equation (2)
            ...
            template_results.append({
                ...
                'brightness_analysis': brightness_differences[i]  # 🔧 YENİ: Parlaklık analizi
            })
...
class CompactPhotometricSimulator:
    ...
    def __init__(self, config_path=None, camera_type='SRC', pds_data_path=None, 
                ...
                global_gain_factor=0.13, gamma_correction=0.43):
    ...
    def compute_sun_strength_photon(
            ...
            *,
            ...
            lambda_eff        = 675e-9,   # m
            pixel_pitch       = 7e-6,     # m
            qe                = 0.07,     # e⁻/photon 0.07 #effective qe
    ...
    def _get_photometric_params(self,
                                ...
                                global_gain_factor: float = 0.18):  # Reference: CORTO validation paper
        """
    ...
    def setup_scene_and_environment(self, utc_time, index, exposure_time_s):
        ...
        photometric_params = self._get_photometric_params(
            ...
            solar_distance_km=solar_distance_km,
            global_gain_factor=self.global_gain_factor  # ✅ Instance variable'ı geçir
        )
        ...
        # ✅ BURAYA EKLEYİN: Light leakage reduction (diffuse_bounces hariç)
        # Reference: Minimizing secondary illumination for low-albedo surfaces (preserving diffuse_bounces=4)
        ...
        self._setup_material_without_albedo_scaling(state, materials, bodies)
    ...
    def _setup_material_without_albedo_scaling(self, state, materials, bodies):
        """Setup materials preserving albedo dynamic range with micro-roughness control
        Reference: Hapke BRDF theory - maintain surface reflectance variations"""
        
        print(f"   🔧 Configuring {len(materials)} materials with micro-roughness θ̄=15°...")
        
        for i, (material, body) in enumerate(zip(materials, bodies), 1):
            if hasattr(corto.Shading, "create_branch_albedo_mix"):
                # Create albedo branch without scaling factor
                corto.Shading.create_branch_albedo_mix(material, state, i)
                
                # Apply micro-roughness control to reduce slope over-brightening
                if material.node_tree:
                    for node in material.node_tree.nodes:
                        # Handle texture nodes
                        if node.type == 'TEX_IMAGE':  # Albedo texture node
                            # Reset any color scaling to preserve dynamic range
                            if hasattr(node, 'color_mapping'):
                                node.color_mapping.brightness = 0.0  # No brightness offset
                                node.color_mapping.contrast = 1.0    # No contrast scaling
                        
                        # ✅ DÜZELTME: Blender 4.x uyumlu BRDF micro-roughness control
                        elif node.type == 'BSDF_PRINCIPLED':  # Main BRDF node
                            # Convert angle to roughness value: θ̄=15° ≈ roughness=0.26
                            micro_roughness = 0.3  # Reduced from ~0.35 (20°) to 0.26 (15°)
                            
                            # ✅ YENİ: Blender version-aware input setting
                            try:
                                # Blender 4.x style
                                if 'Roughness' in node.inputs:
                                    node.inputs['Roughness'].default_value = micro_roughness
                                if 'IOR' in node.inputs:  # Blender 4.x uses IOR instead of Specular
                                    node.inputs['IOR'].default_value = 1.45  # Realistic regolith IOR
                                if 'Metallic' in node.inputs:
                                    node.inputs['Metallic'].default_value = 0.0   # Non-metallic surface
                                
                                print(f"   🔧 Material {i}: micro-roughness set to {micro_roughness:.3f} (θ̄≈15°)")
                                
                            except KeyError as e:
                                # Fallback for different Blender versions
                                print(f"   ⚠️ Material {i}: Blender version compatibility issue: {e}")
                                # Try alternative input names
                                try:
                                    if 'Base Color' in node.inputs:  # At least set basic properties
                                        # Set surface to be dark and non-reflective
                                        node.inputs['Base Color'].default_value = (0.07, 0.07, 0.07, 1.0)
                                    print(f"   🔧 Material {i}: Applied fallback BRDF settings")
                                except:
                                    print(f"   ❌ Material {i}: Could not configure BRDF properties")
                        
                        # ✅ YENİ: Normal map strength control (if present)
                        elif node.type == 'NORMAL_MAP':
                            try:
                                # Reduce normal map strength to soften micro-surface variations
                                normal_strength = 0.7  # Reduced from default 1.0
                                node.inputs['Strength'].default_value = normal_strength
                                print(f"   🔧 Material {i}: normal map strength set to {normal_strength}")
                            except:
                                pass
                        
                        # ✅ YENİ: Bump map control (if present)  
                        elif node.type == 'BUMP':
                            try:
                                # Reduce bump strength to minimize slope over-brightening
                                bump_strength = 0.15  # Reduced from default 0.2
                                node.inputs['Strength'].default_value = bump_strength
                                print(f"   🔧 Material {i}: bump strength set to {bump_strength}")
                            except:
                                pass


    def run_single_simulation_and_validation(self, utc_time, real_img_path, img_filename, index, exposure_time_s):
        """Combined simulation and validation with L2 LUT processing"""
        try:
            # Setup and render
            ...
            if linear_img_path.exists():
                # Apply L2 LUT processing to convert EXR -> L2 PNG
            ...
            return {'index': index, 'utc_time': utc_time, 'img_filename': img_filename, 
                    'real_img_path': real_img_path, 'synthetic_img_path': str(synthetic_img_path), 
                    'processed_img_path': str(processed_img_path) if processed_img_path else None, 
                    'spice_data': convert_numpy_types(spice_data), 'validation_result': validation_result, 
                    'status': 'SUCCESS'}
            
        except Exception as e:
            print(f"Error in L2 simulation: {e}")
            return {'index': index, 'utc_time': utc_time, 'img_filename': img_filename, 
