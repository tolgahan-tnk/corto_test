"""
photometric_template_generator_principled_brdf.py
Photometric template generator with BRDF parameter exploration for Mars, Phobos, and Deimos
"""
...
# —————— Zorla NVIDIA GPU ——————
# Force Cycles to use ONLY the Quadro RTX 4000
try:
    prefs = bpy.context.preferences.addons['cycles'].preferences
    prefs.compute_device_type = 'CUDA'          # veya 'OPTIX' destekliyorsa
    prefs.get_devices()                         # cihaz listesini tazele
    for d in prefs.devices:                     # tüm aygıtlar
        d.use = (d.type == 'CUDA' and           # sadece CUDA türü
                 'Quadro RTX 4000' in d.name)   # ve adı Quadro olanı açık bırak
    bpy.context.scene.cycles.device = 'GPU'     # sahneyi GPU moduna al
    print("🖥️  Cycles device forced →", prefs.compute_device_type,
          "–", [d.name for d in prefs.devices if d.use])
except Exception as e:
    print("⚠️  GPU-forcing snippet failed:", e)
...
class PhotometricTemplateGenerator:
    """Photometric template generator with BRDF parameter exploration"""
    
    def __init__(self, config_path=None, camera_type='SRC', pds_data_path=None,
                 Sun_Blender_scaler=1.0, N_templates=27,
                 mars_specular=0.02, mars_roughness=0.9,
                 deimos_specular=0.02, deimos_roughness=0.9):
        ...
        # -------------------------------------------------------------
        #  Sabit (keşfedilmeyen) BRDF değerleri: 2 = Mars, 3 = Deimos
        #  (Phobos = 1 hâlâ parametre taramasında)
        # -------------------------------------------------------------
        self.static_body_brdf = {
            2: {'specular': mars_specular,   'roughness': mars_roughness},
            3: {'specular': deimos_specular, 'roughness': deimos_roughness},
        }
        # None olanları temizle
        self.static_body_brdf = {
            bid: {k: v for k, v in p.items() if v is not None}
            for bid, p in self.static_body_brdf.items()
        ...
        # Setup BRDF parameter ranges
        self._setup_brdf_ranges()
        
        # Setup output tracking
        self.template_results = []
        self.output_excel_path = None
        
        print(f"✅ Photometric Template Generator initialized")
    ...
    def _setup_brdf_ranges(self):
        """Setup BRDF parameter ranges based on N_templates"""
        # Fixed parameters
        self.brdf_fixed = {
            'metallic': 0.0,  # Non-metallic bodies
            'sheen': 0.0,     # No sheen layer
            'base_color': (1.0, 1.0, 1.0, 1.0)  # White base (modulated by texture)
        }
        
        # Variable parameter ranges
        self.brdf_ranges = {
            'specular': {'min': 0.0, 'max': 2.0},     # Specular reflection strength
            'roughness': {'min': 0.0, 'max': 1.0},    # Surface roughness
            'q_eff': {'min': 0.7, 'max': 1.4}         # Effective quantum efficiency
        }
        
        # Calculate number of steps per parameter
        n_params = len(self.brdf_ranges)
        self.steps_per_param = int(np.round(self.N_templates ** (1.0 / n_params)))
        actual_templates = self.steps_per_param ** n_params
        
        print(f"\n📊 BRDF Parameter Setup:")
        ...
        self.param_values = {}
        for param, range_dict in self.brdf_ranges.items():
    ...
    def _calculate_sun_strength(self, solar_distance_km, q_eff=1.0):
        """Calculate sun strength with quantum efficiency factor
        
        Args:
            solar_distance_km: Distance from sun to Phobos in km
            q_eff: Quantum efficiency factor
    ...
    def _setup_material_with_brdf_params(self, materials, bodies, brdf_params, albedo_factors):
        """Setup materials with specific BRDF parameters"""
        ...
        for i, (material, body) in enumerate(zip(materials, bodies), 1):
            if material.node_tree:
                ...
                if texture_images:
                    ...
                    multiply_node.inputs[1].default_value = albedo_factors.get(i, 0.127)
                    ...
                    # --------------------------
                    #  Gövde-başına BRDF seçimi
                    # --------------------------
                    inp = bsdf_node.inputs
                    inp['Base Color'].default_value = self.brdf_fixed['base_color']
                    inp['Metallic'].default_value   = self.brdf_fixed['metallic']

                    spec_val  = self.static_body_brdf.get(i, {}).get('specular',
                                                                     brdf_params['specular'])
                    rough_val = self.static_body_brdf.get(i, {}).get('roughness',
                                                                     brdf_params['roughness'])

                    if 'Roughness' in inp:
                        inp['Roughness'].default_value = rough_val

                    for spec_name in ('Specular', 'Specular IOR Level'):
                        if spec_name in inp:
                            inp[spec_name].default_value = spec_val
                            break
                    
                    # Try to set sheen if available
                    if 'Sheen' in bsdf_node.inputs:
                        bsdf_node.inputs['Sheen'].default_value = self.brdf_fixed['sheen']
                    elif 'Sheen Weight' in bsdf_node.inputs:
                        bsdf_node.inputs['Sheen Weight'].default_value = self.brdf_fixed['sheen']
                    ...
                    print(f"   ✅ Body {i} material updated: Specular={spec_val:.2f}, "
                          f"Roughness={rough_val:.2f}")
    ...
    def render_template_batch(self, state, env, cam, bodies, sun, materials, tree, spice_data, 
        ...
        for template_idx, brdf_params in enumerate(param_combinations):
            print(f"\n📸 Template {template_idx + 1}/{len(param_combinations)}")
            print(f"   Parameters: {brdf_params}")
            
            try:
                # Update sun energy based on q_eff
                sun_energy = self._calculate_sun_strength(solar_distance_km, brdf_params['q_eff'])
                sun.set_energy(sun_energy)
                
                # Update materials with BRDF parameters
                self._setup_material_with_brdf_params(materials, bodies, brdf_params, self.albedo_factors)
    ...
    def _save_results_to_excel(self):
        ...
        if self.output_excel_path is None:
            ...
            output_dir = Path(self.config['output_path'])
            self.output_excel_path = output_dir / f"template_results_{timestamp}.xlsx"
...
def main(pds_data_path=None, max_simulations=None, camera_type='SRC',
         ...
         mars_specular=0.02, mars_roughness=0.9,
         deimos_specular=0.02, deimos_roughness=0.9):
    """Main function to run photometric template generator
    ...
    """
    print("="*80)
    print("Photometric Template Generator for Mars, Phobos, and Deimos")
    print("="*80)
    ...
    generator = PhotometricTemplateGenerator(
        ...
        mars_specular=mars_specular, mars_roughness=mars_roughness,
        deimos_specular=deimos_specular, deimos_roughness=deimos_roughness
    )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Photometric Template Generator")
    ...
    parser.add_argument("--n_templates", type=int, default=3375, help="Number of templates per image")
    # Sabit Mars / Deimos BRDF değerleri
    parser.add_argument("--mars_specular",   type=float, default=0.02, help="Fixed specular for Mars (default 0.02)")
    parser.add_argument("--mars_roughness",  type=float, default=0.9,  help="Fixed roughness for Mars (default 0.9)")
    parser.add_argument("--deimos_specular", type=float, default=0.02, help="Fixed specular for Deimos (default 0.02)")
    parser.add_argument("--deimos_roughness",type=float, default=0.9,  help="Fixed roughness for Deimos (default 0.9)")
    ...
    template_results = main(
        ...
        mars_specular=args.mars_specular,   mars_roughness=args.mars_roughness,
        deimos_specular=args.deimos_specular, deimos_roughness=args.deimos_roughness
    )
