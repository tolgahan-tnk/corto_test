...
def _record_learned_k(img_stem: str,
    ...
    history.append(int(res['best_ssim_k']))
    best_ssim = float(res.get('best_ssim_score', float('-inf')))
    if best_ssim > float(stats.get('best_ssim', float('-inf'))):
        stats['best_ssim'] = best_ssim
        stats['best_k'] = int(res['best_ssim_k'])
    if (
...
def evaluate_k(real_raw: np.ndarray,
    ...
    _record_learned_k(img_stem, res, learned_k_db)
    return res
